...
def change_formants(sound, f1_target, f2_target, f3_target, max_formant):
    ...
    lpc1 = call(rs1, "To LPC (autocorrelation)", 16, 0.025, 0.005, 50)
    source = call([rs1, lpc1], "Filter (inverse)")

    if df1 != 0:  # Check threshold for F1 adjustment
        call(formant, "Formula (frequencies)", f"if row = 1 then self + {df1} else self fi")
    if df2 != 0:  # Check threshold for F2 adjustment
        call(formant, "Formula (frequencies)", f"if row = 2 then self + {df2} else self fi")
    if df3 != 0:  # Check threshold for F3 adjustment
    ...
    new = rs2.values + hf.values
    
    # Create a new sound with the combined values and the same sampling frequency
    new_sound = parselmouth.Sound(new, sampling_frequency=rs2.sampling_frequency)
    ...
    print(nvf1, nvf2, nvf3)
    print(f1_target, f2_target, f3_target)
    return new_sound
...
result = change_formants(sound, 500.0, 1500.0, 2500.0, 5500)
