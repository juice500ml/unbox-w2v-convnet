...
def _prepare_cfg():
    ...
    parser.add_argument("--base_path", type=str, default="/data/user_data/eyeo2/data/CP/analyze_vowels", help="Base path to the corner vowels directory")
    parser.add_argument("--num_steps", type=int, default=13, help="Number of steps for interpolation")
    parser.add_argument("--spk_info", type=Path, default=Path("praat/timit_spk.csv"), help="spk info for maximum formant frequency (5000 for male, 5500 for female)")
...
def generate_interpolated_sounds(sound_a, sound_b, newfn, num_steps, max_formant):
    ...
    output_folder = os.path.join(f"/data/user_data/eyeo2/data/CP/interpolated_vowels_{num_steps}", folder_name)
...
def select_audio_files(base_path, speaker_id):
    ...
    for sound_a_file, sound_b_file in tqdm(itertools.combinations(sound_files, 2)):
        ...
        # Check if the pair of phonemes is in the valid pairs list
        if (phone_a, phone_b) in pairs or (phone_b, phone_a) in pairs:
    ...
    for speaker_id in tqdm(os.listdir(base_path)):
        ...
        if os.path.isdir(speaker_path):
            ...
            #         # Process both orders: a_b and b_a
                    newfn_ab = f"{sound_a_part[1]}_{sound_a_part[-1]}_{sound_b_part[-1]}"
                    ...
                    newfn_ba = f"{sound_b_part[1]}_{sound_b_part[-1]}_{sound_a_part[-1]}"
